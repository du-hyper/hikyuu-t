...
for stock in sm:
    if stock.market == "SZ" or stock.market == "SH":
        if stock.start_datetime < Datetime("2022-04-19") and stock.last_datetime > Datetime("2023-07-12"):
...
def tomorrowBuy(i):
    for stock in newsm:
        k = stock.get_kdata(Query(start=tradingCalendar[0], end=tradingCalendar[i + 60]))
        if len(k) > 60:
            today_k = k[-1]
            pre_k = k[-2]
            ...
            if volumeRatio > 2.5 and today_k.amount > 20000 and 0 < ROC(CLOSE(k), 1).to_np()[-1] < 7:
                ...
                if _this_open < ma5 < _this_close and _this_close > ma60 and ma5 > ma10 > ma20:
                    ...
                    if today_hsl > 8 and today_hsl < 30:
                        if (i + 62) < len(tradingCalendar):
                            weights = stock.get_weight(tradingCalendar[i + 60], tradingCalendar[i + 62])
                            # 排除分红除权的
                            if len(weights) == 0:
                                capital = LIUTONGPAN(k)[-1]
                                # 流通市值 20亿~500亿
                                if _this_close * capital > 20000 and _this_close * capital < 500000:
                                    market_code[str(tradingCalendar[i + 60])] = stock.market_code
                                    return

def todaySell(i):
    if i < 299 and market_code.get(str(tradingCalendar[i + 58])):
        k = sm[market_code[str(tradingCalendar[i + 58])]].get_kdata(Query(start=tradingCalendar[i + 59]))
        trading_detail[market_code.get(str(tradingCalendar[i + 58]))]["sell_price"] = round(k[0].close, 2)
        trading_detail[market_code.get(str(tradingCalendar[i + 58]))]["sell_time"] = str(tradingCalendar[i + 59])

def buy(i):
    if i < 299 and market_code.get(str(tradingCalendar[i + 59])):
        k = sm[market_code[str(tradingCalendar[i + 59])]].get_kdata(Query(start=tradingCalendar[i + 59]))
        trading_detail[market_code.get(str(tradingCalendar[i + 59]))] = {
            "buy_price": round(k[0].open, 2),
            "buy_time": str(tradingCalendar[i + 59])
        }
